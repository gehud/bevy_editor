...
impl EntityCursor {
    ...
    pub fn eq_cursor_icon(&self, cursor_icon: &CursorIcon) -> bool {
        // If feature custom_cursor is not enabled in bevy_editor, we can't know if it is or not
        // in bevy::window. So we use the wrapper function `as_system` to let bevy::window check its own feature.
