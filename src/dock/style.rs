...
impl Style {
    ...
    pub fn from_egui(style: &egui::Style) -> Self {
        ...
        dock_style.dock_area_padding = Some(Margin {
            left: 4,
            right: 4,
            ..default()
        });

        dock_style.separator.width = 4.0;
        dock_style.separator.color_idle = style.visuals.window_fill;
        dock_style.separator.color_dragged = style.visuals.window_fill;
        dock_style.separator.color_hovered = style.visuals.window_fill;

        dock_style.tab_bar.bg_fill = style.visuals.window_fill;
        dock_style.tab_bar.inner_margin = Margin {
            left: 6,
            right: 6,
            top: dock_style.tab_bar.stroke.width as i8,
            ..default()
        };
        dock_style.tab_bar.corner_radius = CornerRadius {
            nw: 6,
            ne: 6,
            ..default()
        };

        let pane_bg_color = style.visuals.window_fill.linear_multiply(1.25);

        dock_style.tab.active.bg_fill = pane_bg_color;
        dock_style.tab.active_with_kb_focus.bg_fill = pane_bg_color;
        dock_style.tab.focused.bg_fill = pane_bg_color;
        dock_style.tab.focused_with_kb_focus.bg_fill = pane_bg_color;
        dock_style.tab.hovered.bg_fill = pane_bg_color;
        dock_style.tab.inactive.bg_fill = style.visuals.window_fill;
        dock_style.tab.inactive_with_kb_focus.bg_fill = style.visuals.window_fill;

        dock_style.tab.tab_body.bg_fill = pane_bg_color;
